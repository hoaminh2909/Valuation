...
def save_to_csv(data, filename='ticker_data.csv'):
    if os.path.exists(filename):
        ...
        if data["Ticker"].iloc[0] in existing_data["Ticker"].values:
            ...
            data = existing_data.reset_index()
        else:
...
with st.sidebar:
    selected = option_menu(
            menu_title=None,  
            options=["Home", "Trading Portfolio", "Investing Portfolio"],
            icons=["house", "bar-chart", "bar-chart-fill"],  
            menu_icon="cast",  
            default_index=0,  
    ...
    try:
        ...
        if ticker is not None:
            ...
            if bsheet.empty:
                ...
                urlAnalysis = f'https://finance.yahoo.com/quote/{ticker}/analysis/'
                urlStatistics = f'https://finance.yahoo.com/quote/{ticker}/key-statistics?p={ticker}'

                headers = {
                    "User-Agent": "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:89.0) Gecko/20100101 Firefox/89.0"
                }
                response = requests.get(urlAnalysis, headers=headers)
                html_content = response.text
                soup = BeautifulSoup(html_content, 'html.parser')
                section = soup.find('section', attrs={'data-testid':'growthEstimate'})
                # print(section)
                table = section.find('table', attrs={'class':'yf-17yshpm'})
                data_yf = []
                title = []
                ...
                data_yf.append(row_data)
                df_yf = pd.DataFrame(data_yf, columns = title)
                if df_yf.iloc[0,1] != "--":
                    growth_rate1 = float(df_yf.iloc[0,1][:-1])
                ...
                summary, estimation, valuation, watch, guru = st.tabs(
                    ["Summary", 'Discount Rate', "Valuation", "Watchlist", "Financial Analysis"])
                ...
                with valuation:
                    ...
                    soup = BeautifulSoup(html_content, 'html.parser')
                    table = soup.find('table', attrs={'class':'table yf-104jbnt'})
                    da = []
                    title = []
                    ths = table.find_all('th')
                    for th in ths:
                    ...
                    his = {
                        "Ticker": [ticker],
                        "Current Price": [mck.info['currentPrice']],
                        "P/E": [mck.info['trailingPE']],
                        "Forward P/E": [mck.info['forwardPE']],
                        "PEG Ratio": [mck.info['pegRatio']],
                        "P/S": [daf.iloc[0, 1]],
                        "EPS LT Growth Est (%)": [growth_rate1],
                        "Intrinsic Value": [final_intrinsic_value],
...
if selected == "Trading Portfolio":
    ...
    CREDS_FILE = 'valuation-430503-a8771d306d88.json'
    ...
    SERVICE_ACCOUNT_FILE = 'valuation-430503-a8771d306d88.json'
    SCOPES = ['https://www.googleapis.com/auth/spreadsheets']
